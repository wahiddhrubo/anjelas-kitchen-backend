...
export default function Products({ history }) {
  ...
  const styles = {
    ...
    input:
      "placeholder:font-semibold rounded-l-full py-[4px] px-4 placeholder:text-black border-2 border-slate-800 ",
    search:
      "px-8 py-[5.5px]  font-semibold bg-slate-800 rounded-r-full text-white ",
  };
                ...
                <tr key={i._id}>
                  <th className={styles.productTitle}>
                    <img
                      ...
                      alt={i.name}
                    ></img>
                    <span className="ml-3 font-bold text-white">{i.name}</span>
                  </th>
                  ...
                  <td className={styles.tableData}>{i.stocks}</td>
                  <td className={styles.tableData}>{i.category?.join(", ")}</td>
                  <td className={styles.tableData}>{i.tags?.join(", ")}</td>
                  <td className={styles.tableData}>
                    {i.createdAt.split("T")[0]}
                  </td>
                  <td className={styles.tableData}>{i.user.username}</td>
                  <td className={styles.tableData + " text-right "}>
                    <DropDown id={i._id} />
                  </td>
                </tr>
              ))}
